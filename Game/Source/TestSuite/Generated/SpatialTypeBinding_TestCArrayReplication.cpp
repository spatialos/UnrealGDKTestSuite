...
worker::Entity USpatialTypeBinding_TestCArrayReplication::CreateActorEntity(const FString& ClientWorkerId, const FVector& Position, const FString& Metadata, const FPropertyChangeState& InitialChanges, USpatialActorChannel* ActorChannel) const
{
	...
	improbable::unreal::generated::testcarrayreplication::TestCArrayReplicationSingleClientRepData::Data SingleClientTestCArrayReplicationData;
	improbable::unreal::generated::testcarrayreplication::TestCArrayReplicationSingleClientRepData::Update SingleClientTestCArrayReplicationUpdate;
	bool bSingleClientTestCArrayReplicationUpdateChanged = false;
	...
	BuildSpatialComponentUpdate(InitialChanges, ActorChannel, SingleClientTestCArrayReplicationUpdate, bSingleClientTestCArrayReplicationUpdateChanged, MultiClientTestCArrayReplicationUpdate, bMultiClientTestCArrayReplicationUpdateChanged, TestCArrayReplicationHandoverDataUpdate, bTestCArrayReplicationHandoverDataUpdateChanged);
...
void USpatialTypeBinding_TestCArrayReplication::SendComponentUpdates(const FPropertyChangeState& Changes, USpatialActorChannel* ActorChannel, const FEntityId& EntityId) const
{
...
void USpatialTypeBinding_TestCArrayReplication::ReceiveAddComponent(USpatialActorChannel* ActorChannel, UAddComponentOpWrapperBase* AddComponentOp) const
{
...
void USpatialTypeBinding_TestCArrayReplication::ServerSendUpdate_MultiClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::testcarrayreplication::TestCArrayReplicationMultiClientRepData::Update& OutUpdate) const
{
	switch (Handle)
	{
		...
		{
			AActor* Value = *(reinterpret_cast<AActor* const*>(Data));
			const ATestCArrayReplication& ParentValue = *(static_cast<ATestCArrayReplication const*>(ActorChannel->Actor));
			...
			else
			{
				...
				//(const_cast<ATestCArrayReplication&>(ParentValue)).Owner_Context = &SpatialConstants::NULL_OBJECT_REF;
			}
			break;
		}
		case 14: // field_role0
		...
		case 15: // field_instigator0
		{
			...
			else
			{
				...
				//(const_cast<ATestCArrayReplication&>(ParentValue)).Instigator_Context = &SpatialConstants::NULL_OBJECT_REF;
			}
			break;
		}
		...
		case 19: // field_stablynamedarray0
		{
			...
			else
			{
				...
				//(const_cast<ATestCArrayReplication&>(ParentValue)).StablyNamedArray_Context.Add(&SpatialConstants::NULL_OBJECT_REF);
			}
			break;
		}
		...
		case 21: // field_dynamicallycreatedarray0
		{
			...
			else
			{
				...
				//(const_cast<ATestCArrayReplication&>(ParentValue)).DynamicallyCreatedArray_Context.Add(&SpatialConstants::NULL_OBJECT_REF);
			}
			break;
		}
		case 22: // field_dynamicallycreatedarray1
		{
			...
			if (Value != nullptr)
			{
				...
				if ((*ObjectRef) == SpatialConstants::UNRESOLVED_OBJECT_REF)
				{
					// A legal static object reference should never be unresolved.
					check(!Value->IsFullNameStableForNetworking())
					Interop->QueueOutgoingObjectRepUpdate_Internal(Value, ActorChannel, 22);
				}
				else
				{
		...
		case 26: // field_arrayofstructnetserialize1
		{
			...
			Interop->ResetOutgoingArrayRepUpdate_Internal(ActorChannel, 26);
			TSet<const UObject*> UnresolvedObjects;
			TArray<uint8> ValueData;
			FSpatialMemoryWriter ValueDataWriter(ValueData, PackageMap, UnresolvedObjects);
			bool bSuccess = true;
			...
			{
				Interop->QueueOutgoingArrayRepUpdate_Internal(UnresolvedObjects, ActorChannel, 26);
			}
			break;
		}
...
void USpatialTypeBinding_TestCArrayReplication::ServerSendUpdate_Handover(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::testcarrayreplication::TestCArrayReplicationHandoverData::Update& OutUpdate) const
{
}
...
void USpatialTypeBinding_TestCArrayReplication::ReceiveUpdate_MultiClient(USpatialActorChannel* ActorChannel, const improbable::unreal::generated::testcarrayreplication::TestCArrayReplicationMultiClientRepData::Update& Update) const
{
	...
	if (!Update.field_owner0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestCArrayReplication&>(ParentValue)).Owner_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_instigator0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestCArrayReplication&>(ParentValue)).Instigator_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_stablynamedarray0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestCArrayReplication&>(ParentValue)).StablyNamedArray_Context.Add(PackageMap->GetUnrealObjectRefData(ObjectRef));
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_dynamicallycreatedarray0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestCArrayReplication&>(ParentValue)).DynamicallyCreatedArray_Context.Add(PackageMap->GetUnrealObjectRefData(ObjectRef));
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
