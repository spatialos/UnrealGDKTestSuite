...
worker::Entity USpatialTypeBinding_TestStaticComponentReplication::CreateActorEntity(const FString& ClientWorkerId, const FVector& Position, const FString& Metadata, const FPropertyChangeState& InitialChanges, USpatialActorChannel* ActorChannel) const
{
...
void USpatialTypeBinding_TestStaticComponentReplication::SendComponentUpdates(const FPropertyChangeState& Changes, USpatialActorChannel* ActorChannel, const FEntityId& EntityId) const
{
...
void USpatialTypeBinding_TestStaticComponentReplication::ReceiveAddComponent(USpatialActorChannel* ActorChannel, UAddComponentOpWrapperBase* AddComponentOp) const
{
	...
	USpatialTypeBinding_TestComponent* TestComponentTypeBinding = Cast<USpatialTypeBinding_TestComponent>(Interop->GetTypeBindingByClass(UTestComponent::StaticClass()));
	TestComponentTypeBinding->ReceiveAddComponent(ActorChannel, AddComponentOp);
}
...
void USpatialTypeBinding_TestStaticComponentReplication::ServerSendUpdate_SingleClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::teststaticcomponentreplication::TestStaticComponentReplicationSingleClientRepData::Update& OutUpdate) const
{
}

void USpatialTypeBinding_TestStaticComponentReplication::ServerSendUpdate_MultiClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::teststaticcomponentreplication::TestStaticComponentReplicationMultiClientRepData::Update& OutUpdate) const
{
	switch (Handle)
	{
		...
		{
			AActor* Value = *(reinterpret_cast<AActor* const*>(Data));
			const ATestStaticComponentReplication& ParentValue = *(static_cast<ATestStaticComponentReplication const*>(ActorChannel->Actor));
			...
			else
			{
				...
				//(const_cast<ATestStaticComponentReplication&>(ParentValue)).Owner_Context = &SpatialConstants::NULL_OBJECT_REF;
			}
			break;
		}
		case 14: // field_role0
		...
		case 15: // field_instigator0
		{
			...
			else
			{
				...
				//(const_cast<ATestStaticComponentReplication&>(ParentValue)).Instigator_Context = &SpatialConstants::NULL_OBJECT_REF;
			}
			break;
		}
...
void USpatialTypeBinding_TestStaticComponentReplication::ServerSendUpdate_Handover(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::teststaticcomponentreplication::TestStaticComponentReplicationHandoverData::Update& OutUpdate) const
{
}
...
void USpatialTypeBinding_TestStaticComponentReplication::ReceiveUpdate_MultiClient(USpatialActorChannel* ActorChannel, const improbable::unreal::generated::teststaticcomponentreplication::TestStaticComponentReplicationMultiClientRepData::Update& Update) const
{
	...
	if (!Update.field_owner0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestStaticComponentReplication&>(ParentValue)).Owner_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_instigator0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestStaticComponentReplication&>(ParentValue)).Instigator_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
