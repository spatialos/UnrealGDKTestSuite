...
worker::Entity USpatialTypeBinding_TestEnumReplication::CreateActorEntity(const FString& ClientWorkerId, const FVector& Position, const FString& Metadata, const FPropertyChangeState& InitialChanges, USpatialActorChannel* ActorChannel) const
{
	...
	BuildSpatialComponentUpdate(InitialChanges, ActorChannel, SingleClientTestEnumReplicationUpdate, bSingleClientTestEnumReplicationUpdateChanged, MultiClientTestEnumReplicationUpdate, bMultiClientTestEnumReplicationUpdateChanged, TestEnumReplicationHandoverDataUpdate, bTestEnumReplicationHandoverDataUpdateChanged);
...
void USpatialTypeBinding_TestEnumReplication::SendComponentUpdates(const FPropertyChangeState& Changes, USpatialActorChannel* ActorChannel, const FEntityId& EntityId) const
{
...
void USpatialTypeBinding_TestEnumReplication::ReceiveAddComponent(USpatialActorChannel* ActorChannel, UAddComponentOpWrapperBase* AddComponentOp) const
{
...
void USpatialTypeBinding_TestEnumReplication::ServerSendUpdate_MultiClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::testenumreplication::TestEnumReplicationMultiClientRepData::Update& OutUpdate) const
{
	switch (Handle)
	{
		...
		{
			AActor* Value = *(reinterpret_cast<AActor* const*>(Data));
			const ATestEnumReplication& ParentValue = *(static_cast<ATestEnumReplication const*>(ActorChannel->Actor));
...
void USpatialTypeBinding_TestEnumReplication::ServerSendUpdate_Handover(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::testenumreplication::TestEnumReplicationHandoverData::Update& OutUpdate) const
{
}
...
void USpatialTypeBinding_TestEnumReplication::ReceiveUpdate_MultiClient(USpatialActorChannel* ActorChannel, const improbable::unreal::generated::testenumreplication::TestEnumReplicationMultiClientRepData::Update& Update) const
{
	...
	if (!Update.field_owner0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestEnumReplication&>(ParentValue)).Owner_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_instigator0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<ATestEnumReplication&>(ParentValue)).Instigator_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
