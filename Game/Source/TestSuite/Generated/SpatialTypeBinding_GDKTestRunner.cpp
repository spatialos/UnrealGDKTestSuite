...
worker::Entity USpatialTypeBinding_GDKTestRunner::CreateActorEntity(const FString& ClientWorkerId, const FVector& Position, const FString& Metadata, const FPropertyChangeState& InitialChanges, USpatialActorChannel* ActorChannel) const
{
	...
	improbable::unreal::generated::gdktestrunner::GDKTestRunnerSingleClientRepData::Data SingleClientGDKTestRunnerData;
	improbable::unreal::generated::gdktestrunner::GDKTestRunnerSingleClientRepData::Update SingleClientGDKTestRunnerUpdate;
	bool bSingleClientGDKTestRunnerUpdateChanged = false;
	...
	BuildSpatialComponentUpdate(InitialChanges, ActorChannel, SingleClientGDKTestRunnerUpdate, bSingleClientGDKTestRunnerUpdateChanged, MultiClientGDKTestRunnerUpdate, bMultiClientGDKTestRunnerUpdateChanged, GDKTestRunnerHandoverDataUpdate, bGDKTestRunnerHandoverDataUpdateChanged);
...
void USpatialTypeBinding_GDKTestRunner::SendComponentUpdates(const FPropertyChangeState& Changes, USpatialActorChannel* ActorChannel, const FEntityId& EntityId) const
{
...
void USpatialTypeBinding_GDKTestRunner::ReceiveAddComponent(USpatialActorChannel* ActorChannel, UAddComponentOpWrapperBase* AddComponentOp) const
{
...
void USpatialTypeBinding_GDKTestRunner::ServerSendUpdate_SingleClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::gdktestrunner::GDKTestRunnerSingleClientRepData::Update& OutUpdate) const
{
}

void USpatialTypeBinding_GDKTestRunner::ServerSendUpdate_MultiClient(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::gdktestrunner::GDKTestRunnerMultiClientRepData::Update& OutUpdate) const
{
	switch (Handle)
	{
		...
		{
			AActor* Value = *(reinterpret_cast<AActor* const*>(Data));
			const AGDKTestRunner& ParentValue = *(static_cast<AGDKTestRunner const*>(ActorChannel->Actor));
		...
		case 16: // field_testcases0
		{
			...
			Interop->ResetOutgoingArrayRepUpdate_Internal(ActorChannel, 16);
			TSet<const UObject*> UnresolvedObjects;
			...
			{
				Interop->QueueOutgoingArrayRepUpdate_Internal(UnresolvedObjects, ActorChannel, 16);
			}
			break;
		}
...
void USpatialTypeBinding_GDKTestRunner::ServerSendUpdate_Handover(const uint8* RESTRICT Data, const uint8* RESTRICT SourceData, int32 Handle, UProperty* Property, USpatialActorChannel* ActorChannel, improbable::unreal::generated::gdktestrunner::GDKTestRunnerHandoverData::Update& OutUpdate) const
{
}
...
void USpatialTypeBinding_GDKTestRunner::ReceiveUpdate_MultiClient(USpatialActorChannel* ActorChannel, const improbable::unreal::generated::gdktestrunner::GDKTestRunnerMultiClientRepData::Update& Update) const
{
	...
	if (!Update.field_owner0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<AGDKTestRunner&>(ParentValue)).Owner_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_instigator0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
				//(const_cast<AGDKTestRunner&>(ParentValue)).Instigator_Context = PackageMap->GetUnrealObjectRefData(ObjectRef);
				if (NetGUID.IsValid())
				{
					UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
					checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
	...
	if (!Update.field_testcases0().empty())
	{
		...
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			for(int i = 0; i < List.size(); i++)
			{
				...
				{
					FNetworkGUID NetGUID = PackageMap->GetNetGUIDFromUnrealObjectRef(ObjectRef);
					//(const_cast<AGDKTestRunner&>(ParentValue)).TestCases_Context.Add(PackageMap->GetUnrealObjectRefData(ObjectRef));
					if (NetGUID.IsValid())
					{
						UObject* Object_Raw = PackageMap->GetObjectFromNetGUID(NetGUID, true);
						checkf(Object_Raw, TEXT("An object ref %s should map to a valid object."), *ObjectRefToString(ObjectRef));
...
void USpatialTypeBinding_GDKTestRunner::Server_SignalClientReady_SendRPC(worker::Connection* const Connection, void* Parameters, UObject* TargetObject)
{
	auto Sender = [this, Connection, TargetObject]() mutable -> FRPCCommandRequestResult
	{
		...
		UE_LOG(LogSpatialGDKInterop, Verbose, TEXT("%s: Sending RPC: Server_SignalClientReady, target: %s %s"),
			*Interop->GetSpatialOS()->GetWorkerId(),
...
void USpatialTypeBinding_GDKTestRunner::Server_RunTests_SendRPC(worker::Connection* const Connection, void* Parameters, UObject* TargetObject)
{
	auto Sender = [this, Connection, TargetObject]() mutable -> FRPCCommandRequestResult
	{
		...
		// Send RPC
		RPCPayload.set_target_subobject_offset(TargetObjectRef.offset());
		UE_LOG(LogSpatialGDKInterop, Verbose, TEXT("%s: Sending RPC: Server_RunTests, target: %s %s"),
			*Interop->GetSpatialOS()->GetWorkerId(),
			*TargetObject->GetName(),
			*ObjectRefToString(TargetObjectRef));
...
void USpatialTypeBinding_GDKTestRunner::Server_SignalClientReady_OnRPCPayload(const worker::CommandRequestOp<improbable::unreal::generated::gdktestrunner::GDKTestRunnerServerRPCs::Commands::Serversignalclientready>& Op)
{
	auto Receiver = [this, Op]() mutable -> FRPCCommandResponseResult
	{
		...
		if (!TargetNetGUID.IsValid())
		{
			...
			UE_LOG(LogSpatialGDKInterop, Log, TEXT("%s: Server_SignalClientReady_OnRPCPayload: Target object %s is not resolved on this worker."),
				*Interop->GetSpatialOS()->GetWorkerId(),
				*ObjectRefToString(TargetObjectRef));
		...
		// Call implementation.
		UE_LOG(LogSpatialGDKInterop, Verbose, TEXT("%s: Received RPC: Server_SignalClientReady, target: %s %s"),
			*Interop->GetSpatialOS()->GetWorkerId(),
			*TargetObject->GetName(),
			*ObjectRefToString(TargetObjectRef));
		...
		{
			UE_LOG(LogSpatialGDKInterop, Error, TEXT("%s: Server_SignalClientReady_OnRPCPayload: Function not found. Object: %s, Function: Server_SignalClientReady."),
				*Interop->GetSpatialOS()->GetWorkerId(),
...
void USpatialTypeBinding_GDKTestRunner::Server_RunTests_OnRPCPayload(const worker::CommandRequestOp<improbable::unreal::generated::gdktestrunner::GDKTestRunnerServerRPCs::Commands::Serverruntests>& Op)
{
	auto Receiver = [this, Op]() mutable -> FRPCCommandResponseResult
	{
		...
		if (!TargetNetGUID.IsValid())
		{
			// A legal static object reference should never be unresolved.
			checkf(TargetObjectRef.path().empty(), TEXT("A stably named object should not need resolution."));
			UE_LOG(LogSpatialGDKInterop, Log, TEXT("%s: Server_RunTests_OnRPCPayload: Target object %s is not resolved on this worker."),
				*Interop->GetSpatialOS()->GetWorkerId(),
				*ObjectRefToString(TargetObjectRef));
			return {TargetObjectRef};
		}
		...
		// Call implementation.
		UE_LOG(LogSpatialGDKInterop, Verbose, TEXT("%s: Received RPC: Server_RunTests, target: %s %s"),
			*Interop->GetSpatialOS()->GetWorkerId(),
			*TargetObject->GetName(),
			*ObjectRefToString(TargetObjectRef));
		...
		{
			UE_LOG(LogSpatialGDKInterop, Error, TEXT("%s: Server_RunTests_OnRPCPayload: Function not found. Object: %s, Function: Server_RunTests."),
				*Interop->GetSpatialOS()->GetWorkerId(),
				*TargetObject->GetFullName());
		}
