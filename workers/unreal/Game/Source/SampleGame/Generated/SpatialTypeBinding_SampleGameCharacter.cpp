...
const FRepHandlePropertyMap& USpatialTypeBinding_SampleGameCharacter::GetRepHandlePropertyMap() const
{
	...
	if (HandleToPropertyMap.Num() == 0)
	{
		...
		HandleToPropertyMap.Add(44, FRepHandleData(Class, {"TestStructArray"}, COND_SimulatedOnlyNoReplay, REPNOTIFY_OnChanged));
		HandleToPropertyMap.Add(45, FRepHandleData(Class, {"TestStructMovementArray"}, COND_SimulatedOnly, REPNOTIFY_OnChanged));
		HandleToPropertyMap.Add(46, FRepHandleData(Class, {"TestStructStruct", "PS"}, COND_SimulatedOnly, REPNOTIFY_OnChanged));
		HandleToPropertyMap.Add(47, FRepHandleData(Class, {"TestStructStruct", "strength"}, COND_SimulatedOnly, REPNOTIFY_OnChanged));
		HandleToPropertyMap.Add(48, FRepHandleData(Class, {"TestStructStruct", "test"}, COND_SimulatedOnly, REPNOTIFY_OnChanged));
	}
...
void USpatialTypeBinding_SampleGameCharacter::ServerSendUpdate_MultiClient(const uint8* RESTRICT Data, int32 Handle, UProperty* Property, USpatialActorChannel* Channel, improbable::unreal::UnrealSampleGameCharacterMultiClientRepData::Update& OutUpdate) const
{
	switch (Handle)
	{
		...
		case 44: // field_teststructarray
		{
			const TArray<FTestStruct>& Value = *(reinterpret_cast<TArray<FTestStruct> const*>(Data));

			{
				...
				for(int i = 0; i < Value.Num(); i++)
				{
					TArray<uint8> ValueData;
					FMemoryWriter ValueDataWriter(ValueData);
					FTestStruct::StaticStruct()->SerializeBin(ValueDataWriter, reinterpret_cast<void*>(const_cast<FTestStruct*>(&Value[i])));
					List.emplace_back(std::string(reinterpret_cast<char*>(ValueData.GetData()), ValueData.Num()));
				}
				OutUpdate.set_field_teststructarray(List);
			}
			break;
		}
		case 45: // field_teststructmovementarray
		{
...

			{
				...
				OutUpdate.set_field_teststructmovementarray(List);
			}
			break;
		}
		case 46: // field_teststructstruct_ps
		{
			APlayerState* Value = *(reinterpret_cast<APlayerState* const*>(Data));
			...
			if (Value != nullptr)
			{
				...
				if (ObjectRef == SpatialConstants::UNRESOLVED_OBJECT_REF)
				{
					Interop->QueueOutgoingObjectRepUpdate_Internal(Value, Channel, 46);
				}
				else
				{
					OutUpdate.set_field_teststructstruct_ps(ObjectRef);
				}
			}
			else
			{
				OutUpdate.set_field_teststructstruct_ps(SpatialConstants::NULL_OBJECT_REF);
			}
			break;
		}
		case 47: // field_teststructstruct_strength
		{
			float Value = *(reinterpret_cast<float const*>(Data));

			OutUpdate.set_field_teststructstruct_strength(Value);
			break;
		}
		case 48: // field_teststructstruct_test
		{
			int32 Value = *(reinterpret_cast<int32 const*>(Data));

			OutUpdate.set_field_teststructstruct_test(Value);
			break;
		}
	default:
...
void USpatialTypeBinding_SampleGameCharacter::ReceiveUpdate_MultiClient(USpatialActorChannel* ActorChannel, const improbable::unreal::UnrealSampleGameCharacterMultiClientRepData::Update& Update) const
{
	...
	if (!Update.field_teststructarray().empty())
	{
		// field_teststructarray
		uint16 Handle = 44;
		const FRepHandleData* RepData = &HandleToPropertyMap[Handle];
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			uint8* PropertyData = RepData->GetPropertyData(reinterpret_cast<uint8*>(ActorChannel->Actor));
			TArray<FTestStruct> Value = *(reinterpret_cast<TArray<FTestStruct> *>(PropertyData));

			{
				auto& List = (*Update.field_teststructarray().data());
				...
				for(int i = 0; i < List.size(); i++)
				{
					...
					ValueData.Append(reinterpret_cast<const uint8*>(ValueDataStr.data()), ValueDataStr.size());
					FMemoryReader ValueDataReader(ValueData);
					FTestStruct::StaticStruct()->SerializeBin(ValueDataReader, reinterpret_cast<void*>(&Value[i]));
				}
			}
	...
	if (!Update.field_teststructmovementarray().empty())
	{
		// field_teststructmovementarray
		uint16 Handle = 45;
		const FRepHandleData* RepData = &HandleToPropertyMap[Handle];
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				auto& List = (*Update.field_teststructmovementarray().data());
				Value.SetNum(List.size());
				for(int i = 0; i < List.size(); i++)
				{
					{
	...
	if (!Update.field_teststructstruct_ps().empty())
	{
		// field_teststructstruct_ps
		uint16 Handle = 46;
		const FRepHandleData* RepData = &HandleToPropertyMap[Handle];
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			{
				improbable::unreal::UnrealObjectRef ObjectRef = (*Update.field_teststructstruct_ps().data());
				check(ObjectRef != SpatialConstants::UNRESOLVED_OBJECT_REF);
				if (ObjectRef == SpatialConstants::NULL_OBJECT_REF)
				{
					Value = nullptr;
	...
	if (!Update.field_teststructstruct_strength().empty())
	{
		// field_teststructstruct_strength
		uint16 Handle = 47;
		const FRepHandleData* RepData = &HandleToPropertyMap[Handle];
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			Value = (*Update.field_teststructstruct_strength().data());
	...
	if (!Update.field_teststructstruct_test().empty())
	{
		// field_teststructstruct_test
		uint16 Handle = 48;
		const FRepHandleData* RepData = &HandleToPropertyMap[Handle];
		if (bIsServer || ConditionMap.IsRelevant(RepData->Condition))
		{
			...
			Value = (*Update.field_teststructstruct_test().data());
