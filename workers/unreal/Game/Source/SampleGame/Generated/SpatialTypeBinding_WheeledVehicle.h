...
class USpatialTypeBinding_WheeledVehicle : public USpatialTypeBinding
{
  ...
  private:
    ...
    // RPC to sender map.
    using FRPCSender = void (USpatialTypeBinding_WheeledVehicle::*)(worker::Connection* const,
    ...
    void ServerSendUpdate_Migratable(
        const uint8* RESTRICT Data, int32 Handle, UProperty* Property,
        USpatialActorChannel* Channel,
        improbable::unreal::UnrealWheeledVehicleMigratableData::Update& OutUpdate) const;
    ...
    void ReceiveUpdate_Migratable(
        USpatialActorChannel* ActorChannel,
        const improbable::unreal::UnrealWheeledVehicleMigratableData::Update& Update) const;

    // RPC command sender functions.
    void ServerUpdateState_SendCommand(worker::Connection* const Connection,
                                       struct FFrame* const RPCFrame, UObject* TargetObject);

    // RPC command request handler functions.
    void ServerUpdateState_OnCommandRequest(
        const worker::CommandRequestOp<
            improbable::unreal::UnrealWheeledVehicleServerRPCs::Commands::Serverupdatestate>& Op);

    // RPC command response handler functions.
    void ServerUpdateState_OnCommandResponse(
